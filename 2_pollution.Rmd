...
```{r loading-data}
# Your code below:
madrid_pollution3 <- read_tsv(here("data","madrid_pollution.tsv"))
```
...
```{r EBE_pollution}
# Your code below:
EBE_pollution <- madrid_pollution %>% 
  ggplot(aes(x = date, y = EBE)) +
  geom_point() +
  labs(title = "EBE vs Date", x = "Year", y = "EBE Concentration")
EBE_pollution
```

What is your conclusion about the level of EBE over time?
Decresed slightly with less outliers
**Task 3**
...
```{r madrid_pollution}
# Your code below:
madrid_pollution <- madrid_pollution %>% 
  group_by(mnth, year) %>% 
...
madrid_plot<- madrid_pollution %>%
  ...
  theme(axis.text.x = element_text( 
    ...
    size=7, 
    face=3)) +
  labs(title = "EBE vs Date", x = "EBE Concentration", y = "Month") 
madrid_plot
```
...
```{r pollution_2001}
# Your code below:
pollution_2001 <- madrid_pollution3 %>% 
  filter(year == 2001) %>% 
...
```{r pollution_2006}
# Your code below:
pollution_2006 <- madrid_pollution3 %>% 
  filter(year == 2006) %>% 
  select(-date, -year, -mnth) %>% 
  map_df(~max(.x, na.rm = TRUE))
...
```{r subtract-dfs}
# Your code below:
difference = pollution_2006 - pollution_2001
```
...
```{r pollution_diff}
# Your code below:
pollution_diff <- difference %>% 
  pivot_longer(cols = everything(), names_to = "Pollutant", values_to = "2001-2006 difference")
```
...
max_pollution_diff <- pollution_diff %>% 
  arrange(desc(`2001-2006 difference`)) 
