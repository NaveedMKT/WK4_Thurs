...
```{r exercise 01}
IM <- read_csv(here("data", "IM.csv"))
GP <- read_csv(here("data", "GP.csv"))
Both <- full_join(IM,GP)
```
...
```{r exercise 02}
x_id <- c(1:20)
x_value <- rnorm(20, mean = 180, sd = 50)
x_df <- cbind(x_id, x_value)

y_id <- c(1:30)
y_value <- rnorm(30, mean = 10, sd = 2)
y_df <- cbind(y_id, y_value)
```
...
GP2 <-GP %>% 
  mutate(Condition = factor(Condition, labels = c("game", "control"))) %>%
   pivot_longer(cols = GP01_01:GP03_01, names_to = "Name", values_to = "Value")

GP2 %>% 
  group_by(Condition, Name) %>% 
  median(Value) #Does not work
```
